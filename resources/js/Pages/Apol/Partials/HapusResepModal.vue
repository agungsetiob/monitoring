...
watch(
  ...
  (newItem) => {
    if (newItem) {
      ...
      form.noresep = newItem.NORESEP ?? newItem.noresep ?? ''

      // no SEP Apotek (nosepapotek) biasanya = NOAPOTIK, beberapa data bisa NOSJP
      form.nosjp = newItem.NOSJP ?? newItem.nosjp ?? newItem.NOAPOTIK ?? newItem.noapotik ?? ''

      // ref asal sjp: fallback
      form.refasalsjp =
        newItem.REFASALSJP ??
        newItem.refasalsjp ??
        newItem.NOSEP_KUNJUNGAN ??
        newItem.ref_asal_sjp ??
        ''

      // Reset stats and load obat list
      resetDeleteProgress()
      loadObatList()
    }
  }
...
const loadObatList = async () => {
  ...
  try {
    ...
    try {
      ...
      res = await axios.get('/apol/pelayanan/obat/daftar', { params: { nosep: form.nosjp } })
    }
