...
class ApolApiService
{
    ...
    {
        $attempts = [
            'form_data_direct' => function() use ($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir) {
                return $this->attemptFormData($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir);
            },
            'json_wrapped_form' => function() use ($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir) {
                return $this->attemptJsonWrapped($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir);
            },
            ...
            'json_direct' => function() use ($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir) {
                return $this->attemptJsonDirect($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir);
            },
    ...
    /**
     * Method 1: JSON dengan wrapper request - format sesuai dokumentasi BPJS
     */
    private function attemptJsonWrapped($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir)
    {
        ...
        // Format sesuai dokumentasi BPJS - biasanya butuh wrapper "request"
        $requestData = [
            'request' => json_encode([
                'kdppk' => $kdppk,
                'KdJnsObat' => $kdJnsObat,
        ...
        Log::info('APOL Request (JSON Wrapped Form Data)', [
            'data' => $requestData,
            'headers' => array_merge($headers, ['X-signature' => '***masked***'])
        ]);

        $response = Http::withHeaders($headers)
            ->timeout($this->timeout)
            ->asForm()
            ->post($this->baseUrl . '/daftarresep', $requestData);

        return $this->processResponse($response, $ts);
    }

    /**
     * Method 2: Form Data langsung
     */
    private function attemptFormData($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir)
    {
        $ts = $this->makeTimestampSecondsUTC();
        $signature = $this->makeSignature($this->consId, $ts, $this->secretKey);
        ...
        Log::info('APOL Request (Form Data Direct)', [
            'data' => $requestData
        ]);
    ...
    /**
     * Method 3: JSON Body dengan format yang benar
     */
    private function attemptJsonDirect($kdppk, $kdJnsObat, $jnsTgl, $tglMulai, $tglAkhir)
    {
        $ts = $this->makeTimestampSecondsUTC();
        $signature = $this->makeSignature($this->consId, $ts, $this->secretKey);
        ...
        // Coba format seperti RencanaKontrol yang sudah working
        $requestData = [
            'kdppk' => $kdppk,
            'KdJnsObat' => $kdJnsObat,
            'JnsTgl' => $jnsTgl,
        ...
        Log::info('APOL Request (JSON Direct)', [
            'data' => $requestData
        ]);

        $response = Http::withHeaders($headers)
            ->timeout($this->timeout)
            ->json($requestData)  // Use json() method instead of post()
...
{
    $attempts = [
        // Attempt A: DELETE + asForm() + field "request" berisi JSON string (pola umum BPJS)
        'delete_form_request_wrapper' => function () use ($nosjp, $refasalsjp, $noresep) {
            return $this->attemptHapusResepFormWrapped($nosjp, $refasalsjp, $noresep);
        },
        // Attempt B: DELETE + raw JSON tapi Content-Type x-www-form-urlencoded (pola "Documentation Format" yg sering berhasil)
        ...
        // Attempt C: DELETE + application/json murni
        'delete_application_json' => function () use ($nosjp, $refasalsjp, $noresep) {
            return $this->attemptHapusResepJsonDirect($nosjp, $refasalsjp, $noresep);
        },
    ];
...
private function attemptHapusResepFormWrapped(string $nosjp, string $refasalsjp, string $noresep)
{
    $ts        = $this->makeTimestampSecondsUTC();
    $signature = $this->makeSignature($this->consId, $ts, $this->secretKey);
    ...
    // Kirim sebagai field form bernama "request" (JSON string)
    $form = ['request' => json_encode($payload)];

    Log::info('APOL HapusResep (DELETE asForm request=JSON)', ['payload' => $payload]);

    $response = Http::withHeaders($headers)
        ->timeout($this->timeout)
        ->asForm()
        ->delete($this->baseUrl . '/hapusresep', $form);
...
public function getDaftarPelayananObat(string $noSep)
{
    ...
    $headers = [
        ...
        'user_key'    => $this->userKey,
        'Accept'      => 'application/json',
        // GET biasanya tidak perlu Content-Type
    ];

    $base = rtrim($this->baseUrl, '/');

    // urutan percobaan (silakan sesuaikan jika perlu)
    $candidates = [
        "/pelayanan/obat/daftar/" . rawurlencode($noSep),
        "/obat/daftar/" . rawurlencode($noSep),                  // fallback lama
        "/pelayanan/obat/daftar?nosep=" . rawurlencode($noSep),  // fallback pakai query
    ];
    ...
    foreach ($candidates as $path) {
        $url = $base . $path;
        \Log::info('APOL getDaftarPelayananObat TRY', ['url' => $url]);

        $resp = \Illuminate\Support\Facades\Http::withHeaders($headers)
            ->timeout($this->timeout)
            ->get($url);

        // kalau 404, lanjut kandidat berikutnya
        if ($resp->status() === 404) {
            \Log::warning('APOL getDaftarPelayananObat 404 - try next candidate', ['url' => $url]);
            $lastResponse = $resp;
            continue;
        }

        // proses response (akan throw kalau non-2xx selain 404)
        $data = $this->processResponse($resp, $ts);

        // --- NORMALISASI: ambil list obat dari ROOT maupun DETAIL, lalu sinkronkan ---
        ...
        foreach ([$detailList, $rootList] as $lst) {
            ...
            // kalau server kadang kirim object tunggal
            if (is_array($lst) && (
    ...
    // fallback umum
    throw new \Exception('Gagal mengakses endpoint daftar pelayanan obat');
}
...
public function hapusObat(string $noSepApotek, string $noResep, string $kodeObat, string $tipeObat = 'N')
{
    ...
    // Beberapa instansi memetakan path yang berbeda; coba beberapa
    $paths = [
        '/pelayanan/obat/hapus',
        '/obat/hapus',
    ];

    // Urutan attempt (yang paling sering sukses taruh di atas):
    // 1) DELETE + x-www-form-urlencoded + request=<json>
    // 2) DELETE + x-www-form-urlencoded + raw JSON (tanpa key)
    // 3) DELETE + application/json + raw JSON
    // 4) POST  + x-www-form-urlencoded + request=<json>  (beberapa gateway tidak menerima body pada DELETE)
    $attempts = [
        ['method' => 'DELETE', 'ctype' => 'xform_request_kv'],
        ['method' => 'DELETE', 'ctype' => 'xform_raw_json'],
        ['method' => 'DELETE', 'ctype' => 'app_json'],
        ['method' => 'POST',   'ctype' => 'xform_request_kv'],
    ];
    ...
    foreach ($paths as $path) {
        ...
        foreach ($attempts as $att) {
            $client = \Http::withHeaders($headersBase)->timeout($this->timeout);
            $method = $att['method'];
            $ctype  = $att['ctype'];

            try {
                if ($ctype === 'xform_request_kv') {
                    // Content-Type: x-www-form-urlencoded, body: request=<json>
                    $body = http_build_query(['request' => $json]);
                    $client = $client->withHeaders(['Content-Type' => 'application/x-www-form-urlencoded']);
                    $resp = $method === 'DELETE'
                        ? $client->withBody($body, 'application/x-www-form-urlencoded')->delete($url)
                        : $client->withBody($body, 'application/x-www-form-urlencoded')->post($url);

                } elseif ($ctype === 'xform_raw_json') {
                    // Content-Type: x-www-form-urlencoded, body: <json>
                    $client = $client->withHeaders(['Content-Type' => 'application/x-www-form-urlencoded']);
                    ...
                    $resp = $client->withBody($json, 'application/json')->delete($url);
                }

                \Log::info('APOL hapusObat attempt', [
                    'url' => $url, 'method' => $method, 'ctype' => $ctype,
                    'status' => $resp->status(), 'ct' => $resp->header('Content-Type'),
                    'body_preview' => substr((string)$resp->body(), 0, 300) . '...'
                ]);

                // Kalau gateway balas HTML (Request Error page), itu tanda mapping mismatch — coba attempt berikutnya
                $ct = strtolower((string)$resp->header('Content-Type'));
                $looksHtml = (str_contains($ct, 'text/html') || stripos((string)$resp->body(), '<html') !== false);

                if ($looksHtml || !$resp->successful()) {
                    // 404/405/400 HTML => lanjut attempt berikutnya
                    if (in_array($resp->status(), [400, 404, 405]) || $looksHtml) {
                        continue;
                    }
                    // Untuk selain itu, coba parse error JSON kalau ada
                ...
                // Sukses HTTP 2xx — proses seperti biasa (decrypt jika perlu)
                return $this->processResponse($resp, $ts);

            } catch (\Throwable $e) {
                \Log::warning('APOL hapusObat attempt exception: '.$e->getMessage(), compact('url','method','ctype'));
    ...
    return [
        'success'  => false,
        'metaData' => ['code' => '404', 'message' => 'No Mapping Rule matched untuk hapusObat'],
        'response' => ['hint' => 'Cek mapping IGW: DELETE/POST, Content-Type, dan pola body (request=<json>).'],
    ];
}
...
private function tryParseErrorBody(\Illuminate\Http\Client\Response $resp, $ts)
{
    try {
        $data = $resp->json();
        if (is_array($data)) {
